...
# I think it's fair that this goes here and then we do it again? Explore, wrangle, re-explore?
...
birds.subset <- birds %>% #without changing the Family variable below I had errors
  select(ï..Family, Species_name, Genus, F_mass, F_bill,
          ...
          Resource)
# I don't think we will need this one anymore! 
...
```{r more_exploration}
#let's look at the data again, make sure it looks good
dim(birds.family)
...
```{r question_2}
##### we probably want to revisit this because it looks like nothing is significant ####
#test for normality
shapiro.test() #this was giving me issues and I'm sure it's just something silly that I was doing
qqnorm(birds.family$F_tail); qqline(birds.family$F_tail)

#test for equal variance; this is the only testable variable
#common_display is all in the same group, and common_mating is consistent == 2
bartlett.test(birds.family$F_tail ~ birds.family$Common_Resource)

#form the anova
mating.anova <- aov(data = birds.family, F_tail ~ Common_Mating_System*Common_Display*Common_Resource)
summary(mating.anova)
plot(mating.anova)

mating.anova2 <- aov(data = birds.family, F_tail ~ Common_Display + Common_Resource)
summary(mating.anova2)
```
